...
public class UnmuteCommand extends BasePunishmentCommand {
    ...
    public UnmuteCommand(SpigBan plugin) {
        super(plugin);
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!checkPermission(sender, "spigban.unmute")) return true;
        if (args.length < 1) {
            sender.sendMessage(MessageUtil.colorize(
                    getPrefix() + "§cUsage: /unmute <player>"));
            return true;
        }
        ...
        if (!success) sender.sendMessage(MessageUtil.colorize(
                plugin.getMessageUtil().get("unmute-not-muted",
                        Map.of("player", resolvedName))));
        else sender.sendMessage(MessageUtil.colorize(
                plugin.getMessageUtil().get("unmute-success",
                        Map.of("player", resolvedName))));
        return true;
    }
